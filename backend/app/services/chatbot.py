...
CURRENT IMAGE (Analysis ID: {analysis_data.get('analysis_id')}):
...
{detection_details}"""
  ...
  Image {i + 1} (Analysis ID: {analysis.get('analysis_id')}):
  - Timestamp: {analysis.get('timestamp')}
  - Total objects: {analysis.get('count', 0)}
  - Class distribution: {class_dist}
  - Average confidence: {stats.get('average_confidence', 0):.2%}
  - Coverage: {stats.get('coverage_percentage', 0):.2f}%
  - Detections:
{detection_summary}"""
    ...
    def chat(self, user_message, analysis_data=None, all_analyses=None):
